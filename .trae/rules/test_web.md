# Testing
## Unit Tests
Always remember to perform unit tests.
## Additional Tests
- performed when being asked to. 
### Backend & Database Tests
- performed when being asked to "perform backend and database tests".
- after push to github, deployment will run automaticaclly.(no need to deploy manually). Wait for 2-3 mins to test the deployment.
- refer to `/DEPLOYMENT_ISSUE_ANALYSIS.md`. test the backend and database.
- the api and database url can be found in ``
### Website Tests
- performed when being asked to "directly test the website".
- refer to `/TESTING_LOGIC_SUMMARY.md`
- when testing the website, if an ordinary user does not exist, create it first and then test the features